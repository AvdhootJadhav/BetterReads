...
public class HomeController {
    ...
    @Autowired BooksByUserRespository booksByUserRespoitory;
    ...
    public String home(@AuthenticationPrincipal OAuth2User principal, Model model){
        ...
        Slice<BooksByUser> booksSlice = booksByUserRespoitory.findAllById(userId, CassandraPageRequest.of(0, 100));
        List<BooksByUser> booksByUser = booksSlice.getContent();
        booksByUser = booksByUser.stream().distinct().map(book -> {
            String coverImageUrl="/images/no-image.png";
            if(book.getCoverIds() != null && book.getCoverIds().size()>0){
                coverImageUrl = COVER_IMAGE + book.getCoverIds().get(0) + "-M.jpg";
            }
